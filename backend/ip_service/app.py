...
queue = get_queue('ip_queue')

@app.route('/health')
def health():
    return jsonify({"status": "healthy", "service": "ip"})
...
def start_scan():
    ...
    job = queue.enqueue(process_ip_scan, args=(domain, scan_id), job_timeout='1m')
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', debug=True, port=Config.PORT_IP)
