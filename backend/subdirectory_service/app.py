...
queue = get_queue('subdirectory_queue')

@app.route('/health')
def health():
    return jsonify({"status": "healthy", "service": "subdirectory"})
...
def start_scan():
    ...
    job = queue.enqueue(process_subdirectory_scan, args=(domain, scan_id), job_timeout='10m')
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', debug=True, port=Config.PORT_SUBDIRECTORY)
